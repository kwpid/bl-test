...
function refresh()
	...
	for i, item in ipairs(inventory) do
		...
		button.MouseButton1Click:Connect(function()
			...
			local lockButton = popup:FindFirstChild("Lock")
			if lockButton then
				if item.IsLocked then
					lockButton.Text = "🔒"
				else
...
local lockButton = popup:FindFirstChild("Lock")
if lockButton and toggleLockItemEvent then
	lockButton.MouseButton1Click:Connect(function()
		if selectedItemData and selectedItemData.RobloxId then
			toggleLockItemEvent:FireServer(selectedItemData.RobloxId, selectedItemData.SerialNumber)
